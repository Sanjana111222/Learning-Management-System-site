...
const login = async (req,res,next) => {
  try{
    ...
    if (!(user && (await user.comparePassword(password)))) {
      return next(
        new AppError('Email or Password do not match or user does not exist', 400)
      );
    }
...
const getLoggedInUserDetails = async (req, res, _next) => {
...
const logout = async  (req,res,next) => {
 ...
 const getProfile = async (req,res,next) => {
  ...
  {
    return next(new AppError('Failed to fetch the profile',500))
  }
